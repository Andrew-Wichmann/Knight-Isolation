...
match_history = select_match("most_moves_for_player_score_ab", "greedy")
if len(match_history):
    match_history = match_history[0][2:4]
else:
    insert_into_table(values=("most_moves_for_player_score_ab", "greedy", 0, 0))
    match_history = (0,0)
new_match_history = (match_history[0]+record[0], match_history[1]+record[1])
update_table(new_match_history[0], new_match_history[1], "most_moves_for_player_score_ab", "greedy")

DEBUG = False
if DEBUG:
    print("DEBUG:\t\told record pulled from db {}\n\t\tnew record pulled from db {}\n".format(match_history, select_match("most_moves_for_player_score_ab", "greedy")[0][2:4]))
print("The historical match history between most_moves_for_player_score_ab and greedy: {}\n\n\n".format(new_match_history))
...
# Save results in database
match_history = select_match("least_moves_for_other_player_ab", "greedy")
if len(match_history):
    match_history = match_history[0][2:4]
else:
    insert_into_table(values=("least_moves_for_other_player_ab", "greedy", 0, 0))
    match_history = (0,0)
new_match_history = (match_history[0]+record[0], match_history[1]+record[1])
update_table(new_match_history[0], new_match_history[1], "least_moves_for_other_player_ab", "greedy")

if DEBUG:
    print("DEBUG:\t\told record pulled from db {}\n\t\tnew record pulled from db {}\n".format(match_history, select_match("least_moves_for_other_player_ab", "greedy")[0][2:4]))
print("The historical match history between least_moves_for_other_player_ab and greedy: {}\n\n\n".format(new_match_history))